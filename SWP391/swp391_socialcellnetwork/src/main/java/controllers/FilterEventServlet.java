...
import static controllers.FilterAllEventServlet.convertSolar2Lunar;
...
public class FilterEventServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet FilterEventServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet FilterEventServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        out.print("<table>\n"
                ...
                + "                                                            <td>\n"
                + "                                                                <div>\n"
                + "                                                                    <p style=\"font-weight: 600; font-size: 23px\">Myevent</p>\n"
                + "                                                                    <p>" + eventid + "</p>\n"
                + "                                                                </div>\n"
                + "                                                            </td>\n"
                + "                                                        </tr>\n"
                ...
                + "                                                            </td>\n"
                + "                                                            <td>\n"
                + "                                                                "+getInfoByEventId.getDescription()+"\n"
                + "                                                            </td>\n"
                + "                                                        </tr>\n"
                + "                                                        <tr>\n"
                + "                                                            <td>\n"
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        List<EventUser> listEventMonth = CalendarDAO.INSTANCE.loadMyEventByMonthAndYear(currentUser.getUser_id(), month, year);
        if (!listEventMonth.isEmpty()) {
            ...
            for (Map.Entry<String, List<EventUser>> entry : eventsByDay.entrySet()) {
                ...
                for (int i = 0; i < events.size(); i++) {
                    out.println("<div style=\"cursor: pointer;\" onclick=\"openMyEventdetail(" + events.get(i).getEvent_id() + ")\">\n"
