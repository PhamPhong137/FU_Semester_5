 ...
 * @author PC-Phong
 */
public class FilterFamilyEventServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet FilterFamilyEventServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet FilterFamilyEventServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int family_id = Integer.parseInt(request.getParameter("familyid"));

        String year = month_raw.substring(0, 4);
        String month = month_raw.substring(5, 7);
        List<Event> listFamilyEventMonth = CalendarDAO.INSTANCE.loadFamilyEventByMonth(family_id, month, year);
        if (!listFamilyEventMonth.isEmpty()) {

            for (Event e : listFamilyEventMonth) {

                int lunar_day = Integer.parseInt(e.getStart_date().toString().substring(8, 10));
                int lunar_month = Integer.parseInt(e.getStart_date().toString().substring(5, 7));
                int lunar_year = Integer.parseInt(e.getStart_date().toString().substring(0, 4));

                int[] lunarDate = convertSolar2Lunar(lunar_day, lunar_month, lunar_year, 7);

                out.println("<div class=\"event-search-by-month-component\">\n"
                        + "                                <div><p>" + lunar_day + "/" + lunar_month + "</p><span>" + "(" + lunarDate[0] + "/" + lunarDate[1] + ")" + "</span></div>\n"
                        + "                                <div>Title: <span>" + e.getTitle() + "</span></div>\n"
                        + "                                <div>Place: <span>" + e.getPlace() + "</span></div>\n"
                        + "                            </div>");
            ...
            out.println("<div class=\"alert alert-warning alert-dismissible fade show\" role=\"alert\" style=\"width: 100%\">\n"
                    + "\n"
                    + "                                        <h6>Don't have any event on this month </h6>  \n"
                    + "                                    </div>");
        }
